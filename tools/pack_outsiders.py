...
SECTOR_SIZE = 512
NODE_CONTENT_SECTORS = 64
NODE_DISK_SECTORS = 1 + NODE_CONTENT_SECTORS
NODE_CONTENT_SIZE = NODE_CONTENT_SECTORS * SECTOR_SIZE  # 32768
...
def pack():
    ...
    files = sorted(
        [
            f
            for f in os.listdir(OUTSIDERS_DIR)
    ...
    def write_node(index, name, ntype, content, parent_index, children):
        ...
        if ntype == 1 and content_size > 0:
            ...
            img[base_byte + SECTOR_SIZE : base_byte + SECTOR_SIZE + copy_len] = content[
                :copy_len
            ]
